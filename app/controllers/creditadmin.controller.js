...
exports.createCreditadmin = async (req, res) => {
  ...
  people
    .count({ where: { [Op.or]: [{ phone: req.body.phone }, { username: req.body.phone }] } })
    .then(async (user) => {
      if (user <= 0) {
        res.status(400).send({
          status: 400,
      ...
      include: [
        {
          model: people,
          as: "people",
          attributes: ["firstname", "lastname", "username", "phone", "credit"],
        },
        {
          model: user,
          as: "user",
      ...
      include: [
        {
          model: people,
          as: "people",
          attributes: ["firstname", "lastname", "username", "credit", "phone"],
        },
        {
          model: user,
          as: "user",
